...
module.exports.listById = async (event) => {
    ...
    try {
        ...
        if (!userInDb)
            return handlerResponse(400, {}, `${getTitle(userInDb.type)} não encontrado`)
        ...
        if (data.type === userType.USER) {
            ...
            if (!item)
                return handlerResponse(400, {}, `Lodin de ${getTitle(data.type)} não encontrado`)
...
module.exports.create = async (event) => {
    ...
    try {
        ...
        return handlerResponse(201, resultNewUser, `${getTitle(type)} ${name} criado com sucesso`)
    } catch (err) {
...
module.exports.update = async (event) => {
    ...
    try {
        ...
        if (!item)
            return handlerResponse(400, {}, `${getTitle(type)} não encontrado`)
        ...
        return handlerResponse(200, resultUserDb, `${getTitle(type)} ${name} alterado com sucesso`)
    } catch (err) {
        return await handlerErrResponse(err, body)
    }
}
...
module.exports.delete = async (event) => {
    ...
    try {
        ...
        return handlerResponse(200, {}, `${getTitle(userInDb.type)} ${userInDb.name} removido com sucesso`)
    } catch (err) {
        return await handlerErrResponse(err, pathParameters)
    }
}
...
const getTitle = (path, isPlural = false) => {
    switch (path) {
